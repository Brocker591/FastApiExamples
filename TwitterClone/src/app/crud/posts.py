...
async def update_post(post_id: int, db: Session, user_id: int, postmodel: Post):
    post = db.exec(select(Post).where(Post.id == post_id)).first()
    if not post:
        raise HTTPException(status_code=404, detail="Post wurde nicht gefunden")
    ...
    db.commit
    db.refresh(post)
    return {"success": f"Post mit ID {post.id} erfolgreich geupdatet"}
...
async def get_all_post_by_user_id(user_id: int, db: Session):
