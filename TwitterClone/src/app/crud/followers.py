...
async def create_follower(followerModel: FollowerModel, db: Session, user_id: int):
    ...
    followed_user = db.exec(select(User).where(User.id == followerModel.follower_id)).first()
    
    if not current_user or not followed_user:
        raise HTTPException(status_code=404, detail="User or followed user not found")

    already_followed = db.exec(select(Follower).where(Follower.user_id == user_id).where(Follower.follower_id == followerModel.follower_id)).first()

    if already_followed:
        raise HTTPException(status_code=400, detail=f"User mit ID {user_id} folgt dem User mit der ID {followerModel.follower_id} bereits")
...
async def delete_following(follower_id: int, db: Session, user_id: int):
    ...
    follow = db.exec(select(Follower).where(Follower.user_id == user_id).where(Follower.follower_id == follower_id)).first()
